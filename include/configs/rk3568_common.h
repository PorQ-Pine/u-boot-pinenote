...
#define ENV_BOOTMENU_ENTRIES \
	...
	"bootmenu_0=Boot Quill OS=ext4load mmc 0:6 ${kernel_addr_c} /boot/Image.gz; ext4load mmc 0:6 ${fdt_addr_r} /boot/DTB; unzip ${kernel_addr_c} ${kernel_addr_r}; booti ${kernel_addr_r} - ${fdt_addr_r}\0" \
	"bootmenu_1=Boot Recovery OS=sysboot mmc 0:5 any ${scriptaddr} /boot/extlinux/extlinux.conf\0" \
