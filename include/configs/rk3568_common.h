...
#define ENV_BOOTMENU_ENTRIES \
	...
	"bootmenu_0=Boot Quill OS=setenv bootargs root=/dev/ram0 rootfstype=ramfs rdinit=/sbin/init ignore_loglevel rw rootwait earlycon console=ttyS2,1500000n8 fw_devlink=off loglevel=3 vt.global_cursor_default=0; ext4load mmc 0:6 ${kernel_addr_c} /boot/Image.gz; ext4load mmc 0:6 ${fdt_addr_r} /boot/DTB; unzip ${kernel_addr_c} ${kernel_addr_r}; booti ${kernel_addr_r} - ${fdt_addr_r}\0" \
	"bootmenu_1=Boot Quill OS recovery=setenv bootargs root=/dev/ram0 rootfstype=ramfs rdinit=/sbin/init ignore_loglevel rw rootwait earlycon console=ttyS2,1500000n8 fw_devlink=off loglevel=3 vt.global_cursor_default=0 quill_recovery=1; ext4load mmc 0:6 ${kernel_addr_c} /boot/Image.gz; ext4load mmc 0:6 ${fdt_addr_r} /boot/DTB; unzip ${kernel_addr_c} ${kernel_addr_r}; booti ${kernel_addr_r} - ${fdt_addr_r}\0" \
